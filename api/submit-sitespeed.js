export default async function handler(req, res) {
  ...
  try {
    ...
    const rawDomain = clean(body.domain, 253).toLowerCase();
    ...
    if (!/^(?=.{1,253}$)(?:[a-z0-9](?:[a-z0-9-]{0,61}[a-z0-9])?\.)+[a-z]{2,63}$/.test(domain)) return res.status(400).json({ error: 'Please enter a valid domain such as example.com.' });
    ...
    const authHeaders = { apikey: supabaseServiceKey, Authorization: `Bearer ${supabaseServiceKey}` };

    const duplicateByEmail = await fetch(`${base}/rest/v1/website_speed_checks?email=eq.${encodeURIComponent(email)}&created_at=gte.${encodeURIComponent(since)}&select=id&limit=1`, { headers: authHeaders });
    if (!duplicateByEmail.ok) return res.status(500).json({ error: 'Unable to verify submission.' });
    if ((await duplicateByEmail.json()).length) return res.status(429).json({ error: 'A request from this email was received recently. Please try again later.' });

    if (userIp) {
      const duplicateByIp = await fetch(`${base}/rest/v1/website_speed_checks?user_ip=eq.${encodeURIComponent(userIp)}&created_at=gte.${encodeURIComponent(since)}&select=id&limit=1`, { headers: authHeaders });
      if (!duplicateByIp.ok) return res.status(500).json({ error: 'Unable to verify submission.' });
      if ((await duplicateByIp.json()).length) return res.status(429).json({ error: 'A request from this network was received recently. Please try again later.' });
    }
