...
async function geoFor(ip) {
  const db = supa();
  const { data: cached } = await db.from('ip_geo_cache').select('data')
    .eq('ip', ip).gt('fetched_at', new Date(Date.now() - 7 * 864e5).toISOString())
    .maybeSingle();
  ...
  try {
    ...
    const data = {
      ...
      asn: mm ? mm[1] : null,
      isp: mm && mm[2] ? mm[2].trim() : (j.org || null),
    };
    await db.from('ip_geo_cache').upsert({ ip, data }); // upsert refreshes fetched_at
    return data;
  } catch { return null; }
}
...
export default async function handler(req, res) {
  res.setHeader('Access-Control-Allow-Origin', '*'); // public beacon; sendBeacon text/plain needs no preflight
  if (req.method !== 'POST') return res.status(405).end();

  const siteKey = String(req.query.k || '');
  const ip = String(req.headers['x-forwarded-for'] || '').split(',')[0].trim()
    || req.socket?.remoteAddress || '';
  if (!siteKey || !ip) return res.status(400).end();
  ...
  try { if (JSON.stringify(body).length > 8192) return res.status(413).end(); } catch { return res.status(400).end(); }

  const db = supa();
  const { data: site } = await db.from('rum_sites').select('id').eq('site_key', siteKey).maybeSingle();
  if (!site) return res.status(204).end(); // fail silent — never leak which keys are valid

  const geo = await geoFor(ip);
  const { error } = await db.from('rum_events').insert({
    site_id: site.id,
    page_url: String(body.url || '').slice(0, 2048),
    ttfb_ms: m(body.ttfb), fcp_ms: m(body.fcp), lcp_ms: m(body.lcp),
    inp_ms: m(body.inp), cls: m(body.cls, 10), load_ms: m(body.load, 300000),
    connection_type: String(body.ct || '').slice(0, 20) || null,
    device_type: deviceType(String(req.headers['user-agent'] || '')),
    ...(geo || {}),
    payload: body,
  });
  if (error) console.error('[rum] insert failed:', error.message);
