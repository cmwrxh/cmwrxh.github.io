...
const GLOBALPING_TOKEN = process.env.GLOBALPING_API_TOKEN || process.env.GP_API_TOKEN;
...
async function runProbe(location, target) {
  const createResponse = await globalpingFetch('/measurements', {
    ...
    body: JSON.stringify({
      ...
      measurementOptions: { request: { method: 'GET' }, protocol: 'https' },
    }),
  });

  const created = await createResponse.json().catch(() => ({}));
  if (!createResponse.ok || !created.id) {
    throw new Error(created?.error?.message || `Globalping request failed for ${location}: ${createResponse.status}`);
  }
  ...
  while (Date.now() - started < timeoutMs) {
    ...
    if (!resultResponse.ok) throw new Error(`Could not fetch Globalping result for ${location}: ${resultResponse.status}`);
    if (result.status && result.status !== 'in-progress') return { location, raw: result };
    await sleep(1000);
  }
...
export async function runAudit(domain) {
  ...
  const results = await Promise.allSettled(PROBE_LOCATIONS.map((location) => runProbe(location, target)));

  const perLocation = results.map((result, index) => {
    const location = PROBE_LOCATIONS[index];
    return result.status === 'fulfilled'
      ? { location, ...summarizeTiming(result.value.raw) }
      : { location, error: result.reason?.message || 'measurement failed' };
  });

  const successful = perLocation.filter((item) => Number.isFinite(item.ttfbMs));
  const worstTtfb = successful.length ? successful.reduce((a, b) => (a.ttfbMs > b.ttfbMs ? a : b)) : null;
  const bestTtfb = successful.length ? successful.reduce((a, b) => (a.ttfbMs < b.ttfbMs ? a : b)) : null;
  if (!successful.length) throw new Error('No African probe returned a usable HTTP timing result.');
